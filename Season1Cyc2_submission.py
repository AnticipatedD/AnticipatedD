...
class MyPredictor(Predictor):
    ...
    def train(self, features, target):
        ...
        x_fit = x_flat[valid].astype(np.float32, copy=False)
        y_fit = y_flat[valid].astype(np.float32, copy=False)
        ...
        self.scale = np.where(self.scale < 1.0e-10, 1.0, self.scale)
        ...
        if not np.isfinite(y_scale) or y_scale < 1.0e-10:
            y_scale = 1.0
        ...
        gram = gram + np.eye(gram.shape[0], dtype=np.float32) * regularization

        try:
            self.coef = np.linalg.solve(gram, rhs)
        except np.linalg.LinAlgError:
            self.coef = np.linalg.lstsq(gram, rhs, rcond=1.0e-10)[0]
    ...
    def predict(self, features):
        ...
        row_rms = np.maximum(row_rms, 1.0e-10)
        ...
        output = np.zeros_like(target_signal, dtype=np.float32)

        if use_previous:
            active = self.previous_signal.astype(np.float64, copy=True)
        else:
            active = np.zeros(asset_count, dtype=np.float32)
    ...
    def _engineer(self, x_raw):
        ...
        for f in range(feature_count):
            ...
            rank = order.astype(np.float32) / max(asset_count - 1, 1)
            rank = rank - 0.5
        ...
        for f1 in range(feature_count):
            for f2 in range(f1 + 1, feature_count):
                ...
                blocks.append(a * b)
                blocks.append(np.sign(a - b) * np.sqrt(np.abs(a * b) + 1.0e-10))
